...
{{ default (printf "%s.%s.svc.cluster.local:%d" $fullname .Release.Namespace (.Values.controller.service.ports.cluster.number | int)) .Values.publicClusterAddress }}
{{- end -}}
...
{{- $fullname := include "boundary.fullname" . -}}
{{ default (printf "%s-worker.%s.svc.cluster.local:%d" $fullname .Release.Namespace (.Values.worker.service.ports.proxy.number | int)) .Values.publicClusterAddress }}
{{- end -}}
